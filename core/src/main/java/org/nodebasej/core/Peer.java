...
import org.nodebasej.net.StreamConnection;
import org.nodebasej.store.BlockStore;
import org.nodebasej.store.BlockStoreException;
import org.nodebasej.utils.ListenerRegistration;
 ...
 * {@link org.nodebasej.net.AbstractTimeoutHandler} and timeout is automatically disabled (using
 * {@link org.nodebasej.net.AbstractTimeoutHandler#setTimeoutEnabled(boolean)}) once the version
...
public class Peer extends PeerSocketHandler {
     ...
     * {@link org.nodebasej.net.NioClientManager#openConnection(java.net.SocketAddress, StreamConnection)}
     * or
     * {@link org.nodebasej.net.NioClient#NioClient(java.net.SocketAddress, StreamConnection, int)}.</p>
     *
    ...
    /**
     * <p>Construct a peer that reads/writes from the given block chain. Transactions stored in a {@link org.nodebasej.core.TxConfidenceTable}
    ...
    protected void processMessage(Message m) throws Exception {
        ...
        ListenerRegistration<PreMessageReceivedEventListener> registration;
        
        for (Iterator localIterator = preMessageReceivedEventListeners.iterator(); localIterator.hasNext();) { registration = (ListenerRegistration)localIterator.next();
          

          if (registration.executor == Threading.SAME_THREAD) {
            m = ((PreMessageReceivedEventListener)registration.listener).onPreMessageReceived(this, m);
            if (m == null) break;
          }
        }
        if (m == null) { return;
        ...
        } else if ((m instanceof AddressMessage))
        {
          ...
          if (!(m instanceof DarkSendElectionEntryPingMessage))
          {

            if ((m instanceof MasternodeBroadcast)) {
              context.isLiteMode();
...
//            if(!instantSend.mapTxLockVotes.containsKey(item.hash))
    ...
    private void blockChainDownloadLocked(Sha256Hash toHash) {
        ...
        if (cursor != null)
        	...
        	//<HBS>blockLocator.add(params.getGenesisBlock().getHash());
     ...
     * Once the pong is received the value returned by {@link org.nodebasej.core.Peer#getLastPingTime()} is
     * updated.
    ...
    /**
     * Returns the elapsed time of the last ping/pong cycle. If {@link org.nodebasej.core.Peer#ping()} has never
    ...
    /**
     * Returns a moving average of the last N ping/pong cycles. If {@link org.nodebasej.core.Peer#ping()} has never
